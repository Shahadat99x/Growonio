...
import { buildArticleSchema, buildBreadcrumbSchema, buildPageMetadata, type AppLocale } from "@/lib/seo";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string; locale: string }> }): Promise<Metadata> {
  ...
  if (!article) {
    return buildPageMetadata({
      ...
      description: "This insight is unavailable.",
      noIndex: true,
    });
  }
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string; locale: string }> }) {
  ...
  return (
    <div className="min-h-[80vh] pb-24 pt-8">
      <JsonLd
        data={[
          buildArticleSchema(locale as AppLocale, article),
          buildBreadcrumbSchema(locale as AppLocale, [
            { name: locale === "ro" ? "Acasă" : "Home", pathname: "/" },
            { name: t("title"), pathname: "/insights" },
            { name: article.title, pathname: `/insights/${article.slug}` },
          ]),
        ]}
      />
